$(function () {
    function t(t, e) {
        ...
        d3.csv("assets/demo_data/dashboard/traffic_sources.csv", function (a, c) {
    ...
    function e(t, e) {
        ...
        function n() {
            ...
            A.select("path").attr("d", function (t, e) {
                ...
                return g(t.date)
            }), d3.transition(p).select(".d3-axis-vertical").call(x), d3.transition(p).select(".d3-axis-horizontal").attr("transform", "translate(0," + e + ")").call(v), $(window).on("resize", t), $(document).on("click", ".sidebar-control", t)
        }
        var r = d3.select(t)
            , i = {
                top: 5
        ...
        $(".multiselect-container input").uniform({
            radioClass: "choice"
        }), d3.csv("assets/demo_data/dashboard/app_sales.csv", function (t, e) {
            formatted = e, n()
        });
        var h;
    ...
    function a() {
        d3.csv("assets/demo_data/dashboard/app_sales_heatmap.csv", function (t, e) {
            function a() {
            ...
            v.append("rect").attr("class", "heatmap-legend-item").attr("x", function (t, e) {
                ...
                return colors[e]
            }), y.append("text").attr("class", "min-legend-value").attr("x", -10).attr("y", -2).style("text-anchor", "end").style("font-size", 11).style("fill", "#999").text(m), y.append("text").attr("class", "max-legend-value").attr("x", buckets * gridSize + 10).attr("y", -2).style("font-size", 11).style("fill", "#999").text(g), $(window).on("resize", a), $(document).on("click", ".sidebar-control", a)
        })
    }
    ...
    function n(t, e, a) {
        ...
        d3.json("assets/demo_data/dashboard/monthly_sales.json", function (t, m) {
            ...
            }).on("mousemove", g), $(window).on("resize", y), $(document).on("click", ".sidebar-control", y)
        })
    }
    ...
    function r(t, e, a) {
        ...
        d3.json("assets/demo_data/dashboard/monthly_sales.json", function (t, e) {
            ...
            }), $(window).on("resize", p), $(document).on("click", ".sidebar-control", p)
        })
    }
    ...
    function i(t, e, a, n, r, i, s, l) {
        ...
        }, s), $(window).on("resize", d), $(document).on("click", ".sidebar-control", d)
    }
    ...
    function u(t, e, a, n, r, i, s, l, o) {
        ...
        }), n ? d() : c(), $(window).on("resize", u), $(document).on("click", ".sidebar-control", u)
    }
    ...
    function f(t, e) {
            ...
            d3.bullet = function () {
                function i(t) {
                    t.each(function (t, e) {
                        ...
                        F.select("line").attr("y1", h + 3).attr("y2", 7 * h / 6 + 3), F.select("text").attr("y", 7 * h / 6 + 4), M.exit().transition().duration(c).attr("transform", n(v)).style("opacity", 1e-6).remove(), $(window).on("resize", a), $(document).on("click", ".sidebar-control", a)
                    }), d3.timer.flush()
                }
                var o = "left"
                    , d = !1
        ...
        d3.json("assets/demo_data/dashboard/bullets.json", function (t, a) {
            function r() {
            ...
            }, $(window).on("resize", r), $(document).on("click", ".sidebar-control", r)
        })
    }
